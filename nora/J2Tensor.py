...
class J2Material:
    ...
    def update(self, eps_new, ip_point):  # Do I make it possible to give individual ip_points?
        ...
        # print("self.el_Stiff")
        # print(self.el_Stiff.dtype)
        # print("eps_new")
        # print(eps_new)
        # print("self.epsp_hist[ip_point].eps_plastic")
        # print(self.epsp_hist[ip_point].eps_plastic)
        ...
        # eps_el = eps_el.clone().detach().requires_grad_(True)
        ...
        # print("sig_tr.dtype")
        # print(sig_tr.dtype)
       # print('History point', ip_point, ': ', self.epsp_hist[ip_point].eps_plastic)
       # print('epspeq: ', self.epsp_hist[ip_point].eps_p_eq)
        # asdf
        # ([6.0233, 2.2588, 0.0000, 0.0000, 0.0000, 0.0000] # Torch 64
        # [ 6.0233, 2.2588, -4.7684,0.0000, 0.0000, 0.0000] # Torch 32
        ...
        if plasticity:
         ...
         #   print("Applying return mapping scheme")
            ...
            # except:
                # print(" ----------------- Return map failed, findBounds not tested! ----------------- ")
                # gmin, gmax, fmin, fmax = self.y_[ip_point].findBounds( )
                # dgam = self.y_[ip_point].findRootimp ( gmin, gmax, fmin, fmax)
            ...
            # stiff = self.y_[ip_point].compTangStiff(dgam)
            # # TMP JOEP TODO REVERT TO LINE ABOVE:
            # self.stiff = self.y_[ip_point].compTangStiff(dgam)
            # # END TMP

            # Update history
            # deltaG = torch.matmul(torch.t(sig), depsp)
    ...
    class YieldFunc:
        def __init__(self, young, poisson):
            ...
            self.return_map_tol = 1e-7 #1e-10  # 1e-7
            self.maxIter = 100
        ...
        # def compTangStiff(self, dgam):
        #     # Hardening slope
        #     H = self.sigma_C_deriv(self.eps_p_eq)
        #
        #     # Compute ksi for stress and stiffness.
        #     firstterm = torch.matmul(self.P_, self.stress_)[:, 0]
        #     k = torch.matmul(firstterm, self.stress_[:, 0])  # TODO Test if this matmul is correct
        #
        #     # Matrix E
        #     E = self.getEMatrix_(dgam)
        #
        #     # Vector n.
        #     n = torch.matmul(E, torch.matmul(self.P_, self.stress_))
        #
        #     # Alpha.
        #     fac = torch.matmul(firstterm, n[:, 0])
        #     fac += 2. * k * H.item() / (3. - 2. * H.item() * dgam.item())
        #     alpha = 1. / fac
        #
        #     # Assemble the elastoplastic tangent matrix.
        #     dmat3 = E - alpha * torch.matmul(n, torch.t(n))
        #
        #     return dmat3


        # def getAMatrix(self, dgam):
        #     A11 = 3. * ( 1. - self.poisson ) / ( 3. * ( 1. - self.poisson ) + self.young * dgam )
        #     A22 = 1. / (1 + 2. * self.G * dgam)
        #
        #     A1 = (A11 + A22) / 2
        #     A12 = ( A11 - A22 ) / 2
        #
        #     A_mat = torch.zeros((3,3)) #, requires_grad=True)
        #     A_mat[0, 0] = A1
        #     A_mat[1, 0] = A12
        #     A_mat[0, 1] = A12
        #     A_mat[1, 1] = A1
        ...
        # def getEMatrix_(self, dgam):
        #     E11 = 3. * self.young / (3. * (1. - self.poisson) + self.young * dgam)
        #     E22 = 2. * self.G / (1 + 2. * self.G * dgam)
        #     tmp1 = .5 * (E11 + E22)
        #     tmp2 = .5 * (E11 - E22)
        #
        #     E_mat = torch.zeros((3, 3))
        #
        #     E_mat[0, 0] = tmp1
        #     E_mat[0, 1] = tmp2
        #     E_mat[1, 0] = tmp2
